...
#define CHK_RTN_BSEC(x) do{int _rslt = (x); if(_rslt!=0){printf("BSEC Error!%s, %d\n",#x,_rslt);return _rslt;}}while(0)
#define CHK_RTN_BME(x) do{int _rslt = (x); if(_rslt!=0){printf("BME Error!%s, %d\n",#x,_rslt);return _rslt;}}while(0)
...
int BSEC_BME_loop(float* temp, float* pressure, float* humidity,float* VOC)
{
    *temp=INVALID_VALUE; *pressure=INVALID_VALUE; *humidity=INVALID_VALUE;*VOC=INVALID_VALUE;
    ...
    uint32_t del_period = bme68x_get_meas_dur(bme680_settings.op_mode, &new_conf, &gas_sensor) + (heater_conf.shared_heatr_dur * 1000);
    sleep_us(del_period);
    sleep_ms(100);

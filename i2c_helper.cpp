...
float mix_sensors(std::span<float> args)
{
  float result = 0;
  int good_count = 0;
  for (auto arg : args) {
...
void sleep_ms(uint32_t sleep_ms) { std::this_thread::sleep_for(std::chrono::milliseconds(sleep_ms)); }
void sleep_us(uint32_t sleep_us) { std::this_thread::sleep_for(std::chrono::microseconds(sleep_us)); }
int setup_i2C()
{
...
int communicate_I2C(uint8_t device_address,
                    bool write_comm,
                    ...
                    int8_t num_of_bytes)
{
  ...
  if (write_comm == true)
    for (int i = 0; i < num_of_bytes; i++) buff[i + 1] = recv_buff[i];
  ...
  if (write_comm == true) msgs[0].len = 1 + num_of_bytes;
  else msgs[0].len = 1;
  msgs[0].buf = buff;
  ...
  msgset[0].msgs = msgs;
  if (write_comm == true) msgset[0].nmsgs = 1;
  else msgset[0].nmsgs = 2;
